...
void MeshSelector::drawSelectedVertTag(QGLWidget *widget)
{
	if (is_tag_selected_vert_)
	{
		...
		for (int i = 0; i < selectedVert_.size(); i++)
		{
...
void MeshSelector::drawSelectedEdgeTag(QGLWidget *widget)
{
	if (is_tag_selected_edge_)
	{
		...
		for (int i = 0; i < selectedEdge_.size(); i++)
		{
...
void MeshSelector::drawSelectedFaceTag(QGLWidget *widget)
{
	if (is_tag_selected_face_)
	{
		...
		for (int i = 0; i < selectedFace_.size(); i++)
		{
...
void MeshSelector::pickVert(const MyMesh::Point &p)
{
	...
	if (vi<0 || vi>ptr_mesh_->n_vertices())
	{
		return;
	}
	findVertByIdx(vi);
...
void MeshSelector::buildSearchTree()
{
	...
	MyMesh::Point p;
	unsigned nv = ptr_mesh_->n_vertices();
...
int MeshSelector::findEdgeByPoint(const MyMesh::Point &p)
{
	...
	for (MyMesh::FaceHalfedgeIter fhe_it = ptr_mesh_->fh_iter(fh); fhe_it; ++fhe_it)
	{
		...
		if (dis < min_len) { min_len = dis; desiredEdge = ptr_mesh_->edge_handle(fhe_it.handle()).idx(); }
	}
...
int MeshSelector::findFaceByPoint(const MyMesh::Point &p)
{
	...
	{
		tri_count = 0;
		for (MyMesh::FaceVertexIter fv_it = ptr_mesh_->fv_iter(vf_it.handle()); fv_it; ++fv_it)
		{
			tri_p[tri_count] = ptr_mesh_->point(fv_it); ++tri_count;
		}
	...
	if (desiredFace < 0)
	{
		...
		{
			tri_count = 0;
			for (MyMesh::FaceVertexIter fv_it = ptr_mesh_->fv_iter(f_it.handle()); fv_it; ++fv_it)
			{
				tri_p[tri_count] = ptr_mesh_->point(fv_it); ++tri_count;
			}
			if (isPointOnTriangle(tri_p, resultP))
