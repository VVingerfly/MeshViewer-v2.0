...
CArcBall::CArcBall(int winWidth, int winHeight) : m_nWinWidth(winWidth), m_nWinHeight(winHeight)
{
    int i, j;
...
float CArcBall::V3_Norm(HVect v) { return (v.x * v.x + v.y * v.y + v.z * v.z); }
...
float CArcBall::V3_Dot(HVect v1, HVect v2) { return (v1.x * v2.x + v1.y * v2.y + v1.z * v2.z); }
...
void CArcBall::Mouse(HVect vNow) { ball_data.vNow = vNow; }
...
void CArcBall::ShowResult(BallData* ball) { ball->showResult = true; }

/* Stop drawing arc for all drags combined. */
void CArcBall::HideResult(BallData* ball) { ball->showResult = false; }
