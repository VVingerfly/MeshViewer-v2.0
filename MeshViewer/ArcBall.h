...
public:
    ...
    void printBallMatrix()
    {
        printf("BallMatrix (%d x %d)\n", QuatLen, QuatLen);
        for (int i = 0; i < QuatLen; i++) {
            for (int j = 0; j < QuatLen; j++)
                printf("  %.2f", ball_matrix[i][j]);
            printf("\n");
        }
    }
    float* GetBallMatrix()
...
public:
    ...
    void MouseDown(QPoint point) { MouseDown(point.x(), point.y(), m_nWinWidth, m_nWinHeight); }
    void MouseUp(QPoint point) { MouseUp(point.x(), point.y(), m_nWinWidth, m_nWinHeight); }
    void MouseMove(QPoint point) { MouseMove(point.x(), point.y(), m_nWinWidth, m_nWinHeight); }
