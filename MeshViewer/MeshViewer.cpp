...
MeshViewer::MeshViewer(QWidget* parent, MainWindow* mainwindow) : QGLWidget(parent), ptr_mainwindow_(mainwindow)
{
...
void MeshViewer::timerEvent(QTimerEvent* e) { updateGL(); }
...
void MeshViewer::loadMesh()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this, tr("Open mesh file"), tr("../mesh/"),
                                                    tr("OBJ Files (*.obj);;"
                                                       "OFF Files (*.off);;"
                                                       "PLY Files (*.ply);;"
                                                       "STL Files (*.stl);;"
...
{
    QString filename = QFileDialog::getOpenFileName(this, tr("Load Texture"), "../mesh/", tr("Images(*.bmp *.jpg *.png *.jpeg)"));
...
void MeshViewer::saveMesh()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this, tr("Save mesh file"), tr("../mesh/untitled.obj"),
                                                    tr("OBJ Files (*.obj);;"
                                                       "OFF Files (*.off);;"
                                                       "PLY Files (*.ply);;"
                                                       "STL Files (*.stl);;"
...
{
    QString filename = QFileDialog::getSaveFileName(this, ("Save screen as image file"), ("../mesh/untitled.png"),
...
{
    QString info = "A simple triangle mesh processing framework!\n"
...
void MeshViewer::pickMeshElem()
{
    switch (mesh_select_mode_) {
        ...
        case SELECT_VERTEX:
            if (active_mesh_ == SOURCE_ACTIVE)
                ptr_source_model_->selector()->pickVert(current_3d_position_);
            else if (active_mesh_ == TARGET_ACTIVE)
                ptr_target_model_->selector()->pickVert(current_3d_position_);
            break;
        case SELECT_EDGE:
            if (active_mesh_ == SOURCE_ACTIVE)
                ptr_source_model_->selector()->pickEdge(current_3d_position_);
            else if (active_mesh_ == TARGET_ACTIVE)
                ptr_target_model_->selector()->pickEdge(current_3d_position_);
            break;
        case SELECT_FACE:
            if (active_mesh_ == SOURCE_ACTIVE)
                ptr_source_model_->selector()->pickFace(current_3d_position_);
            else if (active_mesh_ == TARGET_ACTIVE)
                ptr_target_model_->selector()->pickFace(current_3d_position_);
            break;
        default:
            break;
    }
